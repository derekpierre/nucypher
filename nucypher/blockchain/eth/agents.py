...
class StakingEscrowAgent(EthereumContractAgent):
    ...
    def get_worker_from_staker(self, staker_address: ChecksumAddress) -> ChecksumAddress:
        worker: str = self.contract.functions.getOperatorFromStaker(staker_address).call()
        return to_checksum_address(worker)

    @contract_api(CONTRACT_CALL)
    def get_staker_from_worker(self, operator_address: ChecksumAddress) -> ChecksumAddress:
        staker = self.contract.functions.stakerFromOperator(operator_address).call()
        return to_checksum_address(staker)

    @contract_api(TRANSACTION)
    def bond_worker(self, transacting_power: TransactingPower, operator_address: ChecksumAddress) -> TxReceipt:
        contract_function: ContractFunction = self.contract.functions.bondOperator(operator_address)
        receipt: TxReceipt = self.blockchain.send_transaction(contract_function=contract_function,
                                                              transacting_power=transacting_power)
        return receipt
    ...
    def release_worker(self, transacting_power: TransactingPower) -> TxReceipt:
        return self.bond_worker(transacting_power=transacting_power, operator_address=NULL_ADDRESS)
    ...
    def staking_parameters(self) -> StakingEscrowParameters:
        parameter_signatures = (
            ...
            'minOperatorPeriods'              # Min amount of periods while a worker can't be changed
        )
